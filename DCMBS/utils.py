...
def load_graph_list(dataset, k):
    if k ==3:
        adj0 = load_graph(dataset, k-2)
        adj_list = [adj0]
        adj1 = load_graph(dataset, k-1)
        adj_list.append(adj1)
        adj2 = load_graph(dataset, k)
        adj_list.append(adj2)

    else:
        adj = load_graph(dataset, k - 1)
        adj_list = [adj]
        adj1 = load_graph(dataset, k)
        adj_list.append(adj1)
...
def load_graph(dataset, k):
    if k:
        path = './graph/{}{}k_graph.txt'.format(dataset, k)
    else:
